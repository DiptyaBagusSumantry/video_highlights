...
def list_folders():
    output_directory = 'output'  # Nama folder utama

    # Cek apakah folder output ada
    if not os.path.exists(output_directory):
        return jsonify({"error": "Folder output tidak ditemukan"}), 404

    # Mendapatkan daftar folder di dalam output
    folders = [name for name in os.listdir(output_directory) if os.path.isdir(os.path.join(output_directory, name))]
